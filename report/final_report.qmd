...
**Hypothesis.** Replace the static homography with a per-frame `classical_lines_v1` estimator (Canny edges [@canny1986edge], probabilistic Hough lines [@duda1972hough; @matas2000probabilistic], RANSAC over candidate line intersections), gated by a hybrid-lock state machine so noisy frames stay at the seed.
...
- **Code.** The implementation repository is referenced in §1.3 (footnote). The headline run is on commit `80b4b60` of branch `feat/quality-fixes-next` (or any descendant on the same branch with the eval framework intact). The configuration file is `configs/experiments/eval_walkover_p3_a1_ball_tracker_net_v1.yaml`.

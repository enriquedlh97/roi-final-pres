...
Table @tbl-regions summarises the five regions and the quality constraint that dominates each.
...
\begin{tikzpicture}[
  ...
  block/.style={rectangle, draw, rounded corners=2pt, minimum width=24mm, minimum height=10mm, align=center, font=\footnotesize},
  decision/.style={diamond, draw, aspect=2, minimum width=20mm, minimum height=8mm, align=center, font=\scriptsize, inner sep=1pt},
  frame/.style={rectangle, draw, dashed, minimum width=16mm, minimum height=8mm, align=center, font=\scriptsize},
  arrow/.style={-{Latex[length=2mm]}, thick}
]
\node[frame] (input) {Input frame $I_t$};
\node[block, right=of input] (sam) {SAM 2\\segmenter};
\node[block, below=8mm of sam] (btn) {TrackNet\\court keypoints};
...
\node[block, above=14mm of gate] (mat) {MatAnyone\\matte $\alpha_t$};
\node[block, right=14mm of gate] (comp) {Inpaint +\\LED-blend};
...
\draw[arrow] (hull.east) -- ++(8mm,0) |- node[above, font=\tiny, near end] {placement quads} (comp.north west);
\draw[arrow] (gate.east) -- node[above, font=\scriptsize] {ramp} (comp);
\draw[arrow] (gate.south) -- ++(0,-3mm) -| node[below, font=\scriptsize, near start] {stay} (comp.south);
...
Each candidate run is scored by the eval framework. The framework reads the run's composited video plus its frozen configuration, auto-discovers placement regions, and computes per-region metrics. Hard gates are listed in Table @tbl-gates.
...
| `corner_accel_p95_px` | $\mathrm{pct}_{95}\, \|\mathbf{c}_{t+1} - 2\mathbf{c}_t + \mathbf{c}_{t-1}\|$ | $< 1.0$ | jittery corner trajectory |
| `quad_area_cv` | $\sigma(A_t) / \mu(A_t)$, $A_t = \mathrm{area}(\text{quad}_t)$ | $< 0.05$ | placement-area flicker |
| `roi_jitter_ratio` | $\frac{\text{frame-diff}_{\text{ROI}}^{\text{comp}}}{\text{frame-diff}_{\text{ROI}}^{\text{orig}}}$ | $\leq 1.05$ | ROI instability vs original |
| `roi_temporal_ssim_mean` | $\mathrm{mean}_t \, \mathrm{SSIM}(\mathrm{ROI}_t, \mathrm{ROI}_{t-1})$ [@wang2004ssim] | $> 0.95$ | temporal coherence in ROI |
| `walkover_logo_visible_pct` | fraction of $\text{quad}_t$ pixels where $|O_t - C_t| > T$, in player-absent rows | $> 0.10$ | logo over-erased during walkover |
| `walkover_occlusion_iou` | $\mathrm{IoU}\!\bigl(\mathbf{1}\{|I_t - C_t| > T\}, \mathbf{1}\{|O_t - L_t^{\text{baked}}| > T\}\bigr)$ inside $\text{quad}_t$ | $> 0.80$ | occlusion accuracy in walkover window |

: Hard gates in the eval framework. Each region must pass all applicable gates to ship; the two walkover gates apply only to the court-floor region. $C_t$ denotes the clean-plate frame and $L_t^{\text{baked}}$ denotes the gold composite's logo at the same frame index. {#tbl-gates}
...
We swept the tolerance $\tau_{\text{lock}} \in \{2, 4, 6, 10, 15, 30, 99999\}$ and ramp speed $\in \{0.3, 1.0, 2.0\}~\text{px/frame}$ over two waves of parallel runs on H200 GPUs. The result is unambiguous (Table @tbl-phase2).
...
Table @tbl-final reports per-region results extracted from the eval framework's machine-readable summary. All four region scorecards pass; the walkover-window evaluation passes both floor-specific gates.
...
@fig-contact shows the forensic sheet at the showpiece frame ($f704$), where the player is directly on top of the floor logo. The six columns are: original; clean court (no logo, no player); our composite; the original-minus-clean delta heatmap (which highlights the player's pixel footprint); a survival heatmap (logo pixels persisting through the matte); and a red leak overlay marking suspected mis-occlusion. On a successful run, columns 4 to 6 are visually quiet: large signal in column 4 means the player is genuinely there, and column 6 should be near-empty.

![Walkover forensic sheet at frame 704, with the player directly on the floor logo. Six columns left-to-right: original broadcast; clean court reference; our composite; original-minus-clean delta; survival heatmap; red leak overlay.](../public/final/walkover_contact.png){#fig-contact width=95%}
...
This work was conducted as part of the Computational Science and Engineering and Data Science Capstone Programme (AC297r) at the Harvard John A. Paulson School of Engineering and Applied Sciences, sponsored by Mitsubishi Electric Research Laboratories (MERL), Cambridge, MA.
...
We thank our MERL industry partners for the problem framing and the regular feedback that shaped the quality bar, and our Harvard programme advisors for the structural guidance that shaped the methodology.

...
author:
  ...
  - name: "Enrique Díaz de León Hicks"
    affiliation: "Harvard University, MS in Data Science"
  - name: "Giovanni Visi"
  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \usepackage{booktabs}
        \usepackage{array}
        % Inject the advisors / industry-partners block on the title page
        % immediately after the abstract, then force the TOC to its own page.
        \usepackage{etoolbox}
        \AtEndEnvironment{abstract}{%
          \vspace{14pt}%
          \begin{center}%
          \small%
          \textbf{Academic advisors:} Pavlos Protopapas, Chris Gumb (Harvard SEAS); Marco Brambilla (Politecnico di Milano).\\[2pt]%
          \textbf{Mitsubishi Electric Innovation Center (MELIC) industry partners:} Zafer Sahinoglu, Alyssa Taliotis, Ismail Ozyigit.%
          \end{center}%
          \clearpage%
        }
        % Override Pandoc's auto-generated author block with a 2-column grid
        % so Harvard sits on the left and Politecnico di Milano on the right,
        % rather than spilling Martina alone onto a second row.
        \AtBeginDocument{%
          \author{%
            \begin{tabular}{cc}
              {\small\textit{Harvard University}} & {\small\textit{Politecnico di Milano}} \\[4pt]
              Raghav Punnam & Giovanni Visi \\[1.2ex]
              Enrique D\'iaz de Le\'on Hicks & Martina Maiorana
            \end{tabular}%
          }%
        }
  html:
...
Our main test case is `melbourne-walking-over-logo.mov`, a thirteen-second sequence from the 2026 Melbourne broadcast at approximately 59 fps (767 frames at 1920 $\times$ 1080). It packs every hard mode into one clip:

- **Five simultaneous placements**: three back-wall banners (objects 1, 2, 5 in our configuration, originally showing a KIA ad), one left-side banner (object 4, originally showing a YoPRO ad), and one court-floor walkover logo (object 3, the painted MELBOURNE wordmark).
...
The court-geometry estimator is the dynamic component of the placement. It computes a per-frame homography that maps the reference court plane to the broadcast image plane.
...
Given $|\mathcal{I}| \geq 5$ image-reference correspondences against a reference court template, we estimate the court-to-image homography $H_t^{\text{net}}$ by RANSAC [@fischler1981ransac] with a five-pixel reprojection threshold. RANSAC selects the homography that maximises the consensus-set size
...
**(1) Erase the original ad.** Inside each placement quad we identify the text-mask region (the original ad pixels) and replace it with a smooth fill matched to the surrounding court colour, plus matched-noise injection so the result does not read as a flat patch:
...
**(3) LED-style brightness re-mapping.** For each pixel inside the placement quad, the warped logo's chrominance is preserved while its luminance is re-mapped against the local surface luminance computed from the inpainted background. Let $Y$ denote the luminance channel. We compute a per-pixel shade map $s_t(p)$ that compares the Gaussian-blurred background luminance at $p$ to the median background luminance inside the placement quad, raised to a configurable strength $s$ and clipped:

$$
\tilde{L}_t^{\text{led}}(p) \;=\; \tilde{L}_t(p) \,\cdot\, \mathrm{clip}\!\left(\frac{Y\,\bigl(G_\sigma * \hat{I}_t^{\text{inpaint}}\bigr)(p)}{\bar{Y}_{\text{quad}, t}}\right)^{\!s},
$$ {#eq-led-blend}

where $\bar{Y}_{\text{quad}, t} = \operatorname{med}_{q \in \text{quad}_t} Y\,\bigl(G_\sigma * \hat{I}_t^{\text{inpaint}}\bigr)(q)$ is the median Gaussian-blurred background luminance inside the placement quad, the gain is clipped to $[\texttt{shade\_clip\_min}, \texttt{shade\_clip\_max}]$, $\sigma$ is set by `shade_blur_ksize`, and $s$ by `shade_strength`. This makes the inserted logo read like a physically painted or printed ad whose brightness tracks the local lighting on the surface: a logo placed in a shaded part of the court darkens, a logo in a bright patch brightens, all without altering the logo's intrinsic colours. Without this step the logo reads as a 2D overlay floating on top of the frame.
...
![Compositor stage walkthrough on a single banner. Left to right: (1) the original broadcast crop containing the original *blua.* ad; (2) the placement quad detected and outlined; (3) the quad rectified to a fronto-parallel rectangle in the reference surface frame; (4) the replacement *Ferrari* logo composed in the same rectified frame; (5) the new logo warped back through the inverse homography into the original perspective and blended via the LED-style luminance re-mapping (@eq-led-blend). The same pipeline runs per frame for every placement region.](../public/final/compositor_steps.png){#fig-compositor-steps width=100%}
...
**Layer 2: structured visual rubric.** A thirteen-dimension rubric scored against paired top-original / bottom-composite crop strips, with the original ad as the comparison anchor. Catches qualitative regressions that numerical metrics miss (halo, edge reflex, texture match, player contact shadow). Scores are produced one candidate at a time and recorded in the run's machine-readable summary.
...
$\mathrm{IoU}$ of $\mathbf{1}\{|Y(I_t) - Y(C_t)| > T\}$ vs $\mathbf{1}\{|Y(O_t) - Y(L_t^{\text{gold}})| > T\}$ inside $\text{quad}_t$, with $T = 20$ on 8-bit luminance &
$> 0.80$ &
...
Hard gates in the eval framework. Each region must pass all applicable gates to ship; the two walkover gates apply only to the court-floor region. $C_t$ denotes the clean-plate frame and $L_t^{\text{gold}}$ denotes the gold composite's logo at the same frame index. The SSIM definition follows @wang2004ssim.
...
For each candidate run the eval framework writes paired strips per region: the top row is the unmodified original broadcast (the real physical ad), the bottom row is our composite at the same frames. The original is always present as the comparison anchor, because absolute scoring drifts toward "looks fine" without it.
...
LED-style luminance re-mapping & Toggled via the compositor's \texttt{blend\_mode} knob during the v68 saturation sweep (P1-C series): switching \texttt{led} to \texttt{screen} and \texttt{lum\_strength} from 0.0 to 0.3 produced visible but not measured changes in painted-on appearance. & Not run quantitatively \\
...
![Back-wall banners at three distinct placement positions (objects 1, 2, 5), captured at the same frame index. Top row: original broadcast with the original KIA ad. Bottom row: our composite with the inserted Red Bull logo.](../public/final/crops_back_3banners.png){#fig-back width=95%}

@fig-left shows the left-side banner.

![Left side banner. Top: original broadcast with the original YoPRO ad. Bottom: our composite with the Red Bull logo. The mild halo at letter edges is the residual texture-match limitation.](../public/final/crops_left_1.png){#fig-left width=60%}
...
**Cost decomposition.** The marginal H200 inference cost for the final 13-second clip is $4.54/\text{hr} \times 286\,\text{s} / 3600\,\text{s/hr} \approx 0.36$ USD. This is the only cost that scales linearly with clip volume on a fixed deployment; it is also the smallest component of the actual per-clip cost. Three other components are not amortised in that number:
...
**Real-time throughput.** A ten-fold speed-up is required for live broadcast. The pipeline is per-frame so each stage can be optimised independently: SAM 2, the court-keypoint detector, the hybrid-lock state machine, MatAnyone, and the compositor. Each stage is independently amenable to lower-precision inference, batched processing, or distillation to a smaller model.
...
The pipeline code (eval framework, hybrid-lock state machine, BallTrackerNet port, compositor, walkover-window detector) is available in the companion repository referenced in §1.3. Foundation-model checkpoints (SAM 2, MatAnyone, TennisCourtDetector) are downloaded at run time from their respective upstream releases. The demonstration clip is not redistributed by us.

# Acknowledgements {.unnumbered}

This work was a collaboration between the Harvard John A. Paulson School of Engineering and Applied Sciences, the Politecnico di Milano, and the Mitsubishi Electric Innovation Center (MELIC), undertaken within the Computational Science and Engineering and Data Science Capstone Programme (AC297r) at Harvard SEAS.

The author team is split between Harvard and Politecnico di Milano. Raghav Punnam is an MS student in Computational Science and Engineering and Data Science at Harvard University. Enrique Díaz de León Hicks is an MS student in Data Science at Harvard University. Giovanni Visi and Martina Maiorana are MS students in Computer Science and Engineering at Politecnico di Milano.

We thank our academic advisors for the structural guidance that shaped the methodology and the writing: Pavlos Protopapas and Chris Gumb at Harvard SEAS, and Marco Brambilla at Politecnico di Milano. We thank our industry partners at MELIC, Zafer Sahinoglu, Alyssa Taliotis, and Ismail Ozyigit, for the original problem framing, the iteration feedback that shaped the quality bar, and the sustained collaboration throughout the project.
